...
<body>
    ...
    <br />
    <br />
    <h3 style="text-align: center; font-size: 12pt; margin: 0;">SURAT PERNYATAAN PENGAJUAN SPP-LS</h3>
    <center>
        Nomor:
    ...
    <p style="text-align: justify; text-indent: 1cm;">
        Sehubungan dengan Surat Permintaan Pembayaran Langsung (SPP-LS) yang kami ajukan sebesar <strong>Rp.
            {{ number_format($spp->belanja_ls->nilai, 2, ',', '.') }}</strong>
        (<i>{{ Str::title(Terbilang::make($spp->belanja_ls->nilai)) }}
            @if (fmod($spp->belanja_ls->nilai, 1) != 0)
                Koma {{ Str::title(Terbilang::make(fmod($spp->belanja_ls->nilai, 1) * 100)) }}
            @endif Rupiah
    ...
    <ol style="text-align: justify;">
        <li>Jumlah Langsung (LS) tersebut di atas akan dipergunakan untuk keperluan guna membiayai kegiatan yang
            akan kami laksanakan sesuai DPA BLUD-SKPD.</li>
        <li>Jumlah Langsung (LS) tersebut tidak akan digunakan untuk membiayai pengeluaran-pengeluaran yang
            menurut ketentuan yang berlaku harus dilakukan dengan pembayaran Uang Persediaan/Ganti Uang
            Persediaan/Tambah Uang Persediaan (UP/GU/TU).</li>
    </ol>
    <p style="text-align: justify; text-indent: 1cm;">
        Demikian Surat Pernyataan ini dibuat untuk melengkapi persyaratan pengajuan SPP-LS BLUD SKPD kami.
    </p>
