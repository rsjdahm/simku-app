@php
    function getBelanjaRkaPd($bulan)
    {
        return $new_belanja_rka_pd = App\Models\Anggaran\BelanjaRkaPd::withSum(
            [
                'bukti_gu' => function ($q) use ($bulan) {
                    return $q
                        ->whereMonth('tanggal', $bulan)
                        ->whereNotNull('tanggal')
                        ->where('status', App\Enums\Penatausahaan\StatusPosting::Posting);
                },
            ],
            'nilai',
        )
            ->withSum(
                [
                    'belanja_ls' => function ($q) use ($bulan) {
                        return $q
                            ->whereMonth('tanggal', $bulan)
                            ->whereNotNull('tanggal')
                            ->where('status', App\Enums\Penatausahaan\StatusPosting::Posting);
...
<table style="width: 100%; border-collapse: collapse; border: 0;">
    <tr>
        ...
        <td>
            ...
            <div style="text-align: center;">
                Per Bulanan Tahun {{ Carbon\Carbon::parse($tgl_start)->year }}
            </div>
        </td>
    </tr>
</table>
    ...
    <table class="table-sm table-bordered table-hovered table">
        <thead class="bg-primary text-white">
            <tr>
                ...
                <th colspan="12" class="text-center">REALISASI</th>
            ...
            <tr>
                @for ($i = 1; $i <= 12; $i++)
                    <th>{{ Carbon\Carbon::create()->month($i)->isoFormat('MMMM') }}</th>
                @endfor
            </tr>
        </thead>
        <tbody>
                    ...
                    <tr class="font-weight-bold bg-warning">
                        ...
                        @for ($i = 1; $i <= 12; $i++)
                            <td style="text-align: right;">
                                @php
                                    $jumlah_realisasi_per_kelompok_bulanan =
                                        getBelanjaRkaPd($i)
                                            ->whereIn('rek_sub_rincian_objek_id', $rek_sub_rincian_objek->whereIn('rek_rincian_objek_id', $rek_rincian_objek->whereIn('rek_objek_id', $rek_objek->whereIn('rek_jenis_id', $rek_jenis->where('rek_kelompok_id', $_rek_kelompok->id)->pluck('id'))->pluck('id'))->pluck('id'))->pluck('id'))
                                            ->sum('belanja_ls_sum_nilai') +
                                        getBelanjaRkaPd($i)
                                            ->whereIn('rek_sub_rincian_objek_id', $rek_sub_rincian_objek->whereIn('rek_rincian_objek_id', $rek_rincian_objek->whereIn('rek_objek_id', $rek_objek->whereIn('rek_jenis_id', $rek_jenis->where('rek_kelompok_id', $_rek_kelompok->id)->pluck('id'))->pluck('id'))->pluck('id'))->pluck('id'))
                                            ->sum('bukti_gu_sum_nilai');
                                @endphp
                                {{ number_format($jumlah_realisasi_per_kelompok_bulanan, 2, ',', '.') }}</td>
                        @endfor
                        <td style="text-align: right;">
                            @php
                    ...
                    @foreach ($rek_jenis->where('rek_kelompok_id', $_rek_kelompok->id)->all() as $_rek_jenis)
                        ...
                        <tr style="font-weight: bold; background: #dedede">
                            ...
                            @for ($i = 1; $i <= 12; $i++)
                                <td style="text-align: right;">
                                    @php
                                        $jumlah_realisasi_per_jenis_bulanan =
                                            getBelanjaRkaPd($i)
                                                ->whereIn('rek_sub_rincian_objek_id', $rek_sub_rincian_objek->whereIn('rek_rincian_objek_id', $rek_rincian_objek->whereIn('rek_objek_id', $rek_objek->where('rek_jenis_id', $_rek_jenis->id)->pluck('id'))->pluck('id'))->pluck('id'))
                                                ->sum('belanja_ls_sum_nilai') +
                                            getBelanjaRkaPd($i)
                                                ->whereIn('rek_sub_rincian_objek_id', $rek_sub_rincian_objek->whereIn('rek_rincian_objek_id', $rek_rincian_objek->whereIn('rek_objek_id', $rek_objek->where('rek_jenis_id', $_rek_jenis->id)->pluck('id'))->pluck('id'))->pluck('id'))
                                                ->sum('bukti_gu_sum_nilai');
                                    @endphp
                                    {{ number_format($jumlah_realisasi_per_jenis_bulanan, 2, ',', '.') }}</td>
                            @endfor
                            <td style="text-align: right;">
                                @php
                        ...
                        @foreach ($rek_objek->where('rek_jenis_id', $_rek_jenis->id)->all() as $_rek_objek)
                            ...
                            <tr style="font-weight: bold; background: #ececec">
                                ...
                                @for ($i = 1; $i <= 12; $i++)
                                    <td style="text-align: right;">
                                        @php
                                            $jumlah_realisasi_per_objek_bulanan =
                                                getBelanjaRkaPd($i)
                                                    ->whereIn('rek_sub_rincian_objek_id', $rek_sub_rincian_objek->whereIn('rek_rincian_objek_id', $rek_rincian_objek->where('rek_objek_id', $_rek_objek->id)->pluck('id'))->pluck('id'))
                                                    ->sum('belanja_ls_sum_nilai') +
                                                getBelanjaRkaPd($i)
                                                    ->whereIn('rek_sub_rincian_objek_id', $rek_sub_rincian_objek->whereIn('rek_rincian_objek_id', $rek_rincian_objek->where('rek_objek_id', $_rek_objek->id)->pluck('id'))->pluck('id'))
                                                    ->sum('bukti_gu_sum_nilai');
                                        @endphp
                                        {{ number_format($jumlah_realisasi_per_objek_bulanan, 2, ',', '.') }}</td>
                                @endfor
                                <td style="text-align: right;">
                                    @php
